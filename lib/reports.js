...
class Reports {
  ...
  /**
   * Weekly report
   * Kept for backwards compatibility in the API, forcing the report details search to automagically
   * limit the data with a start_date of one week ago.
   */
  async weekly(workspaceId, params) {
    params = { ...params, start_date: dayjs().subtract(1, 'week').format('YYYY-MM-DD') };
